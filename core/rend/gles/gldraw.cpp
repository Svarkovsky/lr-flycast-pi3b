...
static inline bool SameGPState(const PolyParam* a, const PolyParam* b)
{
	if (a->texid != b->texid) return false;
	if (a->pcw.Texture != b->pcw.Texture) return false;
	if (a->pcw.Offset != b->pcw.Offset) return false;
	if (a->pcw.Gouraud != b->pcw.Gouraud) return false;
	if (a->pcw.Shadow != b->pcw.Shadow) return false;
	if (a->pcw.Volume != b->pcw.Volume) return false;

	if (a->tsp.UseAlpha != b->tsp.UseAlpha) return false;
	if (a->tsp.IgnoreTexA != b->tsp.IgnoreTexA) return false;
	if (a->tsp.ShadInstr != b->tsp.ShadInstr) return false;
	if (a->tsp.FogCtrl != b->tsp.FogCtrl) return false;
	if (a->tsp.DstInstr != b->tsp.DstInstr) return false;
	if (a->tsp.SrcInstr != b->tsp.SrcInstr) return false;
	if (a->tsp.FilterMode != b->tsp.FilterMode) return false;
	if (a->tsp.ClampU != b->tsp.ClampU) return false;
	if (a->tsp.ClampV != b->tsp.ClampV) return false;
	if (a->tsp.FlipU != b->tsp.FlipU) return false;
	if (a->tsp.FlipV != b->tsp.FlipV) return false;
	if (a->tsp.ColorClamp != b->tsp.ColorClamp) return false;
	if (a->tsp.MipMapD != b->tsp.MipMapD) return false;

	if (a->isp.CullMode != b->isp.CullMode) return false;
	if (a->isp.DepthMode != b->isp.DepthMode) return false;
	if (a->isp.ZWriteDis != b->isp.ZWriteDis) return false;

	if (a->tcw.PixelFmt != b->tcw.PixelFmt) return false;
	if (a->tcw.MipMapped != b->tcw.MipMapped) return false;
	if (a->tcw.ScanOrder != b->tcw.ScanOrder) return false;
	if (a->tcw.PalSelect != b->tcw.PalSelect) return false;

	if (a->tileclip != b->tileclip) return false;
...
static void DrawList(const List<PolyParam>& gply, int first, int count)
{
	...
	const PolyParam* last_params = nullptr;

	while (count-- > 0)
	{
		if (params->count > 2) /* this actually happens for some games. No idea why .. */
		{
			if (!last_params || !SameGPState(last_params, params))
			{
