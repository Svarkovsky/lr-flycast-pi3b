...
{
	0x00000, 0x00001, 0x00002, 0x00006, 0x00016, 0x00056, 0x00156, 0x00556, 0x01556, 0x05556, 0x15556
};
static const u32 OtherMipPoint[11] =
{
	0x00003, 0x00004, 0x00008, 0x00018, 0x00058, 0x00158, 0x00558, 0x01558, 0x05558, 0x15558, 0x55558
};
...
// Быстрый per-frame кэш хэшей VRAM для исключения повторных расчетов XXH32
struct VramHashCacheEntry {
	const void* tex_ptr;
	u32 frame;
	u32 hash;
};
static VramHashCacheEntry vram_hash_cache[512];
...
u32 BaseTextureCacheData::ComputeVramHash()
{
	...
	// Проверяем per-frame кэш
	size_t cache_idx = (((uintptr_t)this) >> 4) & 511;
	if (vram_hash_cache[cache_idx].tex_ptr == this && vram_hash_cache[cache_idx].frame == FrameCount)
		return vram_hash_cache[cache_idx].hash;

	u32 h = XXH32(&vram[start], end - start, 7);
	vram_hash_cache[cache_idx] = { this, (u32)FrameCount, h };
	return h;
}
...
void BaseTextureCacheData::Update()
{
	...
	::palette_index = this->palette_index;
	::vq_codebook = &vram[vq_codebook];
