...
struct ChannelEx
{
	...
	// Оптимизация: 1 умножение вместо 2
	__forceinline SampleType InterpolateSample()
	{
		u32 fp = step.fp;
		return s0 + FPMul((s32)(s1 - s0), (s32)fp, 10);
	}
	...
	__forceinline bool Step(SampleType& oLeft, SampleType& oRight, SampleType& oDsp)
	{
		...
		else
		{
			...
			oDsp = FPMul(sample, logtable[ds], 11);
...
void AICA_Sample32()
{
	...
	{
		// Быстрый пропуск отключенных каналов
		if (!Chans[ch].enabled)
			continue;
	...
	// Вынос инвариантов из 32-сэмплового цикла
	const bool is_mono = CommonData->Mono;
	const u32 mvol = CommonData->MVOL;
	const s32 mvol_val = volume_lut[mvol];
	const bool dac18b = CommonData->DAC18B;
	const bool cdda_muted = (settings.aica.CDDAMute != 0);
	const u32 efsdl_16 = dsp_out_vol[16].EFSDL;
	const u32 efpan_16 = dsp_out_vol[16].EFPAN;
	const u32 efsdl_17 = dsp_out_vol[17].EFSDL;
	const u32 efpan_17 = dsp_out_vol[17].EFPAN;

	for (int i = 0; i < 32; i++)
	{
		SampleType mixl = mxlr[i * 2 + 0];
		SampleType mixr = mxlr[i * 2 + 1];
		...
		if (!cdda_muted)
		{
			VOLPAN(EXTS0L, efsdl_16, efpan_16, mixl, mixr);
			VOLPAN(EXTS0R, efsdl_17, efpan_17, mixl, mixr);
		}
		...
		mixl = (s32)FPMul((s64)mixl, mvol_val, 15);
		mixr = (s32)FPMul((s64)mixr, mvol_val, 15);
