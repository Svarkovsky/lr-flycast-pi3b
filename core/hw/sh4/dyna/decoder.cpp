...
{
	reg_mach, reg_macl, reg_pr, reg_sgr,
	NoReg, reg_fpul, reg_fpscr, NoReg,
	NoReg, NoReg, NoReg, NoReg,
	NoReg, NoReg, NoReg, reg_dbr,
};

static const Sh4RegType CREGS[] =
{
	reg_sr, reg_gbr, reg_vbr, reg_ssr,
	reg_spc, NoReg, NoReg, NoReg,
	...
	case DM_WriteM:
		{
			...
			if ((s32)e<0)
			{
				if (rs1._reg!=rs2._reg && !mmu_enabled())
				{
	...
	case DM_DIV0:
		{
			...
			else
			{
				...
				{
					verify(!state.cpu.is_delayslot);
					Emit(shop_xor, mk_reg(reg_sr_T), mk_reg(div_som_reg3), mk_reg(div_som_reg2));
					Emit(shop_and, mk_reg(reg_sr_T), mk_reg(reg_sr_T), mk_imm(1 << 31));

					Emit(shop_div32s, mk_reg(div_som_reg1), mk_reg(div_som_reg1), mk_reg(div_som_reg2), 0, mk_reg(div_som_reg3), mk_reg(div_som_reg3));
					
					Emit(shop_and, mk_reg(reg_temp), mk_reg(div_som_reg1), mk_imm(1));
					Emit(shop_sar, mk_reg(div_som_reg1), mk_reg(div_som_reg1), mk_imm(1));
					Emit(shop_or, mk_reg(reg_sr_T), mk_reg(reg_sr_T), mk_reg(reg_temp));

					Emit(shop_div32p2, mk_reg(div_som_reg3), mk_reg(div_som_reg3), mk_reg(div_som_reg2), 0, mk_reg(reg_sr_T));

					Emit(shop_and, mk_reg(reg_sr_T), mk_reg(reg_sr_T), mk_imm(1));
		...
		case NDO_NextOp:
			{
				...
				else
				{
					...
					if (!mmu_enabled())
					{
						// FPU и целочисленные опкоды корректно списывают квант времени
...
#if defined(LOW_END)
	// Виртуальный андерклокинг SH4 200MHz -> ~145MHz (множитель 1.375x)
	// Блок списывает на 37.5% больше виртуальных тактов, снижая объем работы CPU на 27%
	blk->guest_cycles = (blk->guest_cycles * 11) / 8;
#endif
