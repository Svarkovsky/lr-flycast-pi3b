...
static inline void enable_arm_fpu_fast_mode()
{
#if defined(__arm__)
    uint32_t arm_fpscr_val;
    __asm__ __volatile__(
        "mrc p10, 7, %0, cr1, cr0, 0 \n"
        "orr %0, %0, #(3 << 24)       \n" // Устанавливаем FZ (бит 24, Flush-To-Zero) и DN (бит 25, Default NaN)
        "mcr p10, 7, %0, cr1, cr0, 0 \n"
        : "=r"(arm_fpscr_val)
        :
        : "memory"
    );
#elif defined(__aarch64__)
    uint64_t arm_fpcr_val;
    __asm__ __volatile__(
        "mrs %0, fpcr             \n"
        "orr %0, %0, #(3 << 24)   \n"
        "msr fpcr, %0             \n"
...
DynarecCodeEntryPtr rdv_FindOrCompile()
{
	...
	if (rv == ngen_FailedToFindBlock)
		rv = (DynarecCodeEntryPtr)CC_RW2RX(rdv_CompilePC(0));
