...
#define group_EN() { TileClipMode(data->pcw.User_Clip);}
	static Ta_Dma* TACALL ta_main(Ta_Dma* data,Ta_Dma* data_end)
	{
		do
		{
			...
			case ParamType_Object_List_Set:
				{
					INFO_LOG(PVR, "Unsupported list type: ParamType_Object_List_Set");
					data+=SZ32;
				}
				break;
...
//
// Быстрая побитовая валидация IEEE-754 вершин (без библиотечных вызовов isnan/fabsf)
// 0x7E800000u соответствует ~1.7e37f. Все NaNs, Infs и переполненные float имеют (val & 0x7FFFFFFF) >= 0x7E800000
//
static inline bool is_vertex_inf(const Vertex& vtx)
{
	const u32 ux = (const u32&)vtx.x & 0x7FFFFFFFu;
	const u32 uy = (const u32&)vtx.y & 0x7FFFFFFFu;
	const u32 uz = (const u32&)vtx.z;

	if (ux >= 0x7E800000u || uy >= 0x7E800000u || (uz & 0x7FFFFFFFu) >= 0x7E800000u)
		return true;

	return ((const s32&)uz) < 0;
}
...
void FillBGP(TA_context* ctx)
{
	...
	float scale_x= (SCALER_CTL.hscale) ? 2.f:1.f;
	for (int i=0;i<3;i++)
	{
	...
	reinterpret_cast<u32&>(bg_depth) &= 0xFFFFFFF0;
